...
function extractModelSchemaMongoose(file) 
{
    ...
    let { schemaProcessed } = extractParameters(mongooseSchema);
    let schema = preParser(schemaProcessed);
...
{
    // Strip everything before module.exports
    let sequelizeSchema = file.match(/sequelize\.define\s*\(\s*{[\s\S]*/s)[0].match(/^[^;]*/gm)[0];
    sequelizeSchema = sequelizeSchema.substring(sequelizeSchema.indexOf("{")).replace(/^\s*\/\/.*/gm, "");
    let { schemaProcessed } = extractParameters(sequelizeSchema);
    let schema = preParser(schemaProcessed);
    while(schema.length > 0) 
    {
        try 
    ...
    for (let controller of controllers) 
    {
        ...
        try 
        {
            ...
            for (let controllerFunction of file.split(",")) 
            {
                ...
                if (["", "{", "}", " "].includes(controllerFunction)) 
                {
                    continue;
                }
