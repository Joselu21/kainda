...
The services folder contains all the services of the project, which are basically utility functions that can be used by any part of the application. The auth.service.js file is the only service currently autogenerated because it's necessary for routes, which contains a function to verify the JWT token and add the user to the request object exported directly by the kainda internal module, so you can override it easily if you need to.
